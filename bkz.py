...
def bkz_kernel(arg0, params=None, seed=None):
    ...
    if KoN == "k" or KoN == "K":
        filepath = "./kyber_data/Matrix_{}".format(sys.argv[1])
    elif KoN == "n" or KoN == "N":
        filepath = "./Nttru_data/Matrix_{}".format(sys.argv[1])
    ...
    MM = GSO.Mat(A, float_type="dd",
                 U=IntegerMatrix.identity(A.nrows, int_type=A.int_type),
                 UinvT=IntegerMatrix.identity(A.nrows, int_type=A.int_type))

    g6k = Siever(MM, params)
    if dont_trace:
        tracer = dummy_tracer
    else:
        tracer = SieveTreeTracer(g6k, root_label=("svp-challenge", d), start_clocks=True)
    ...
    for blocksize in blocksizes:
        ...
        for t in range(tours):
            ...
            if verbose:
                ...
                fn = open("./logs/Matrix_{}_{}_{}".format(sys.argv[1],str(blocksize),str(time.time())), "w")
                fn.write(str(M.B))
                fn.close()
...
if __name__ == '__main__':
    bkz_tour()
    '''
    python bkz.py 246 --bkz/betas 70:86:5 --threads 18 --verbose --KoN k --challenge_seed 246  
    python bkz.py 256 --bkz/betas 45:65:5 --threads 18 --verbose --KoN k --challenge_seed 256  --bkz/extra_d4f 12 --bkz/jump 3
    '''
